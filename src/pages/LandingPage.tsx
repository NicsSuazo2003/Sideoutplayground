...
export function LandingPage() {
  ...
  // Fetch today's availability for About tab preview
  useEffect(() => {
    if (activeTab === 'about') {
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {availability.map(slot => {
                          ...
                          return (
                            <motion.tr key={slot.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                              className={`border-b border-white/5 transition-colors ${isSelected ? 'bg-[#7CFC00]/10' : slot.isAvailable ? 'hover:bg-white/3' : 'opacity-50'}`}>
                              ...
                              <td className="py-2.5 px-3">
                                {isSelected ? (
                                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-[#7CFC00]/20 text-[#7CFC00]"><Check size={10} /> Selected</span>
                                ) : slot.isAvailable ? (
                                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-green-500/10 text-green-400">Vacant</span>
                                ) : (
                                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-white/5 text-white/30"><Lock size={10} /> Booked</span>
                                )}
                              </td>
                              <td className="py-2.5 px-3 text-right">
                                <span className={`font-semibold ${isSelected ? 'text-[#7CFC00]' : slot.isAvailable ? 'text-white/60' : 'text-white/20'}`}>₱{slot.price || pricePerHour}</span>
                              </td>
                              <td className="py-2.5 px-3 text-right">
                                {isSelected ? (
                                  <button onClick={() => deselectSlot(slot.id)} className="text-xs text-red-400 hover:text-red-300 font-semibold">Remove</button>
                                ) : slot.isAvailable ? (
                                  <button onClick={() => handleSlotClick(slot)} className="px-3 py-1 rounded-lg bg-[#7CFC00] text-black text-xs font-bold hover:bg-[#7CFC00]/80 transition-colors">Book</button>
                                ) : (
