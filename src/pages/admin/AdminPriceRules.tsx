...
export function AdminPriceRules() {
    ...
    try {
      if (editingId) {
        await updatePriceRule(editingId, {
          ...
          pricePerHour: parseFloat(form.pricePerHour), priority: parseInt(form.priority),
        });
  ...
  const handleEdit = (rule: PriceRule) => {
    setForm({
      ...
      pricePerHour: String(rule.pricePerHour), priority: String(rule.priority),
    });
  ...
  return (
    <div className="space-y-5 max-w-2xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-white">Price Rules</h1>
          <p className="text-white/50 text-sm mt-1">Peak pricing for specific days and times</p>
        </div>
        <Button variant="neon" size="sm" onClick={() => { resetForm(); setShowForm(true); }}>
          <Plus size={14} /> Add Rule
        </Button>
      </div>

      {showForm && (
        <motion.form initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSave} className="glass-card p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-white font-bold">{editingId ? 'Edit Rule' : 'New Rule'}</h3>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-sm text-white/80 block mb-1.5">Day</label>
      ...
      <div className="space-y-3">
        {rules.length === 0 ? (
          <div className="glass-card p-8 text-center text-white/40 text-sm">No price rules yet. Base court price applies to all slots.</div>
        ) : (
          rules.map(rule => (
            <div key={rule.id} className="glass-card p-4 flex items-center justify-between">
              <div>
                <div className="text-white font-semibold text-sm">{rule.name}</div>
                <div className="text-white/40 text-xs">
                  {rule.dayOfWeek} · {rule.startTime}–{rule.endTime} · ₱{rule.pricePerHour}/hr · Priority {rule.priority}
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => handleEdit(rule)} className="text-xs text-[#7CFC00] hover:underline">Edit</button>
